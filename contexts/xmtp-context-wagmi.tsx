...
export const XMTPProviderWagmi = ({ children }: { children: ReactNode }) => {
  ...
  // Create signer using domain_space's exact pattern
  ...
  // Simplified connection logic like domain_space
  ...
  const revokeInstallations = useCallback(async () => {
    ...
    try {
      ...
      // Try to extract inbox ID using static methods
      try {
      ...
      toast.error(`XMTP revocation failed: ${errorMessage}`);
    } finally {
      setIsLoading(false);
    }
    ...
    try {
      ...
      return {
        ...
        installations: installations.map((installation, index) => ({
          ...
          isCurrent: index === installations.length - 1,
  ...
  const revokeSpecificInstallations = useCallback(async (installationIds: string[]) => {
    ...
    try {
      ...
      toast.error(`Failed to revoke installations: ${errorMessage}`);
    } finally {
      setIsLoading(false);
    }
  }, [address, signer, connectXmtpCore]);
  ...
  const clearLocalData = useCallback(() => {
    ...
    try {
      ...
      console.log('✅ Local XMTP data cleared');
      toast.success('XMTP local data cleared. Please refresh and try again.');
    } catch (err) {
  ...
  // Simple connect to XMTP like domain_space
  const connectXmtp = useCallback(async () => {
    ...
    try {
      ...
      toast.error(`XMTP connection failed: ${errorMessage}`);
    } finally {
      setIsLoading(false);
    }
  }, [address, connectXmtpCore]);

  // Don't auto-connect by default to prevent installation limit issues
  // Users will need to manually connect XMTP
  useEffect(() => {
    if (!address) {
      setClient(null);
      setError(null);
...
export const useXMTPContextWagmi = () => {
  const context = useContext(XMTPContext);
  if (context === undefined) {
    throw new Error("useXMTPContextWagmi must be used within an XMTPProviderWagmi");
  }
  return context;
};